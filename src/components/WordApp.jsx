...
function WordApp() {
  ...
  const populateVoices = () => {
    ...
    return voices.filter(voice => voice.gender === 'female' || voice.name.toLowerCase().includes('female'));
  };
      ...
      {wordData.slice(0, visibleWordsCount).map(word => (
        <div className='border-bottom mb-4' key={word.id}>
          <p className=''>
            ...
            <span onClick={() => readWordAloud(word.word)} style={{ cursor: 'pointer', marginLeft: '5px' }}>
